...
<script src=\"".site_url()."lib/plupload/plupload.min.js\"></script>
...
var uploader = new plupload.Uploader({
	...
	init: {
		...
		toastr.error(err.message, 'Error ' + err.code);
		if(err.code == 600 || err.code == -600 ) {
		toastr.error('".get_option('maxup','200')."mb', '"._lang("Max allowed size is")."', {timeOut: 10000});
		} else {
...
$data =  '<div id="vibe-error" class="hide msg-warning mtop20 mbot20 mleft20 mright20"></div>
   <div class="vibe-upload row" style="margin:50px 0">			
	<div id="AddVid" class="text-center mtop20 left20 right20 ">
   <div class="vibeprogress">    
	...
	<div id="dumpvideo" class="text-center mtop20">
	 <a id="pickfiles" class="btn btn-primary mtop20" href="javascript:;"><i class="icon-plus"></i> '._lang("Choose a video").'</a>
	...
	<input type="text" id="tags" data-validate="false" name="tags" class="tags form-control" value="">
	</div>
	</div>
	</div>
	</div>';
