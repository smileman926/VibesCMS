	...
	*/
	...
	class ezSQLcore
	{
		...
		*/
		...
		{
		 global $cache_name;
         if(isset($cache_name) && not_empty($cache_name)) {
			// The would be cache file for this query
			$cache_file = $this->cache_dir.'/'.$cache_name.'.cache';
			$cache_name = null; //reset
           } else  {
		   // The would be cache file for this query
			$cache_file = $this->cache_dir.'/'.md5($query).'.cache';
		   }
		...
		*/
		...
		$full_dir = ABSPATH.'/cache/full/*';
		$log = array();
		/* Remove sql cache */
		$clist = glob($cache_dir."{*.cache*}", GLOB_BRACE);
		if($clist) {
		...
		if($full){
		/* Remove full cache */
		$flist = glob($full_dir."{*.html*}", GLOB_BRACE);
		if($flist) {
		...
		/* Remove empty trash */
		$tlist = glob($full_dir."{[^\.]}", GLOB_BRACE);
		if($tlist) {
      ...
      chmod($filename, 0777);
      if (unlink($filename)){
      return '<div class="msg-win">'.$filename.' removed.</div>';
      } else {
      return '<div class="msg-warning">'.$filename.' was not removed. Check server permisions for "unlink" function.</div>';
}
} else {
 return '<div class="msg-warning">'.$filename.' was not removed. Seems to be missing.</div>';
}
}
