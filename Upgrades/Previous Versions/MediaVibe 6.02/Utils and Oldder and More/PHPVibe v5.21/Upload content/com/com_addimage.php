...
//var_dump($_FILES);
	...
	$allowedExtArray = array('jpg', 'png', 'gif','jpeg','bmp');	# Set allowed file types
	$imageQuality    = 100;
...
//Do the sql insert
$db->query("INSERT INTO ".DB_PREFIX."images (`privacy`,`media`,`category`, `pub`,`source`, `user_id`, `date`, `thumb`, `title`, `tags` , `views` , `liked` , `description`, `nsfw`) VALUES 
('".intval(_post('priv'))."','3','".intval(_post('categ'))."','".intval(get_option('videos-initial'))."','".$source."', '".user_id()."', now() , '".$thumb."', '".toDb(_post('pic-title')) ."',  '".toDb(_post('tags'))."', '0', '0','".toDb(_post('pic-desc'))."','".toDb(_post('nsfw'))."')");	
$doit = $db->get_row("SELECT id from ".DB_PREFIX."images where user_id = '".user_id()."' order by id DESC limit 0,1");
add_activity('8', $doit->id);
$error .= '<div class="msg-info mtop20 mright20">'._post('pic-title').' '._lang("created successfully.").' <a href="'.site_url().me.'">'._lang("Manage media.").'</a></div>';
...
$data =  $error.'
<h1 class="block full mtop20 mbot20">'._lang("Share a picture").'</h1>	
   <div class="clearfix vibe-upload right20">			
	<div class="row well right20 block">
...
<label class="control-label" for="inputFile">'._lang("Choose picture:").'</label>
...
<input type="file" id="imgInp" name="play-img" required=""/>
	...
	<div class="controls">
	<input type="text" id="title" name="pic-title" class="form-control col-md-12" required="" value="">
	</div>
	</div>';	
	$data .= '
