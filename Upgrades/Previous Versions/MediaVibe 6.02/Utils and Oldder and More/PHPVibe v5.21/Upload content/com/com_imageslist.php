...
$options = DB_PREFIX."images.id,".DB_PREFIX."images.title,".DB_PREFIX."images.date,".DB_PREFIX."images.user_id,".DB_PREFIX."images.thumb";
/* Define list to load */
$interval = '';
if(_get('sort'))
{
...
case mostcom:
	    ...
	    $vq = "select ".DB_PREFIX."images.id,".DB_PREFIX."images.title,".DB_PREFIX."images.user_id,".DB_PREFIX."images.thumb,".DB_PREFIX."images.views,".DB_PREFIX."images.liked,".DB_PREFIX."images.duration,".DB_PREFIX."images.nsfw, ".DB_PREFIX."users.name as owner, ".DB_PREFIX."users.avatar , count(a.object_id) as cnt FROM ".DB_PREFIX."em_comments a LEFT JOIN ".DB_PREFIX."images ON a.object_id LIKE CONCAT('%', ".DB_PREFIX."images.id) LEFT JOIN ".DB_PREFIX."users ON ".DB_PREFIX."images.user_id = ".DB_PREFIX."users.id WHERE ".DB_PREFIX."images.liked > 0 and ".DB_PREFIX."images.date < now() and pub > 0 group by a.object_id order by cnt desc ".this_limit();
		$active = mostcom;
		break;		
case mostviewed:
