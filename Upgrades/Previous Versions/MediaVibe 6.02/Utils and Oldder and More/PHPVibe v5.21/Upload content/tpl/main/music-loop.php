...
foreach ($songs as $song) {
			...
			$watched = (is_watched($song->id)) ? '<span class="badge badge-dark badge-sm">'._lang("Listened").'</span>' : '';
            ...
            $wlater = (is_user()) ? '<a class="laterit btn btn-icon btn-outline btn-primary pv_tip" data-toggle="tooltip" data-placement="right" title="'._lang("Listen later").'" href="javascript:Padd('.$song->id.', '.later_playlist().')"><i class="icon icon-plus"></i></a>' : '';
			echo '
<li id="song-'.$song->id.'" class="list-group-item song">
                  <div class="media">
                    <div class="media-left">
                    <a class="song" href="'.$url.'">
                        <img src="'.thumb_fix($song->thumb, true, 60, 60).'" alt="">
            		</a>
                    </div>
                    <div class="media-body">
                      <div class="pull-right songs-quicks">
                        '.$liked.$wlater.' <a href="'.$url.'" class="btn btn-icon btn-success btn-outline"><i class="icon icon-play"></i></a>
                      </div>
                      <div>
                       <span> <a href="'.$url.'"><strong>'.$full_title.'</strong></a> </span>'.$watched.'
