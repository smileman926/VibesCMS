...
$tq= "select ".$options.", '".$profile->name."' as owner FROM ".DB_PREFIX."videos WHERE views > 0 and pub > 0 and date < now() and user_id ='".$profile->id."' AND MONTH(date) = MONTH(CURDATE( )) ORDER BY views DESC ".this_offset(2);
$trending = $db->get_results($tq);
if($trending) {
echo '<div id="SearchResults" class="loop-content phpvibe-video-list vTrends">'; 
...
echo '<br style="clear:both"><div class="moreP full text-center">+ '.number_format($vd->nr - $md->nr - 6).' '._lang("more videos").'</div>';
echo '</div>';
}
/* Music */
if($md->nr > 0) {
echo '<h4 class="loop-heading"><span>'._lang("Songs").' ('.number_format($md->nr).')</span> <a href="'.site_url().'forward/ums-'.$profile->id.'/" class="btn btn-default btn-xs pull-right"><i class="icon icon-headphones"></i> '._lang("Play all").'</a></h4>';	
...
if(($md->nr - 6) > 0) {
echo '<div class="moreP full text-center">+ '.number_format($md->nr - 6).' '._lang("more songs").'</div>';
}
echo '</div>';
}
if($id->nr > 0) {
...
echo '</div>';
if(($id->nr - 6) > 0) {
echo '<div class="moreP full text-center">+ '.number_format($id->nr - 6).' '._lang("more pictures").'</div>';
}
echo '</div>';
}
