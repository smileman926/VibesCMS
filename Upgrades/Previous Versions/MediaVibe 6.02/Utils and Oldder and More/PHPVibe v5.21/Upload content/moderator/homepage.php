...
if($categories) {
$tt =array(""=>"[Video] " ,"1" =>"[Videos] ","2" =>"[Songs] ", "3"=>"[Images] ");	
...
?>
		 ...
		 <div class="box-element">
 					<div class="box-head-light"><i class="icon-play"></i><h3>New Channel block</h3></div>
					<div class="box-content">
<form id="validate" class="form-horizontal styled" action="<?php echo admin_url('homepage'); ?>" enctype="multipart/form-data" method="post">
	...
	<?php $c = $cachedb->get_results("Select id,name FROM ".DB_PREFIX."users order by views desc limit 0,1000 ");
	if($c) {
	foreach($c as $cl)  { ?>
	<option value="<?php echo $cl->id;?>"><?php echo _html($cl->name);?> </option>
	<?php }
	}
	?>
	</select>
...
if($boxes_sql) {
$bt = array(""=>"Media","2"=>"Media","1"=>"Html","3"=>"Playlist","4"=>"Channel");
...
<form id="validate" class="form-horizontal styled" action="<?php echo admin_url('homepage'); ?>" enctype="multipart/form-data" method="post">
	...
	<select data-placeholder="Select playlist" name="queries" id="queris" class="select validate[required]" tabindex="2">
	<?php $p = $cachedb->get_results("select ".DB_PREFIX."playlists.*, ".DB_PREFIX."users.name as user from ".DB_PREFIX."playlists LEFT JOIN ".DB_PREFIX."users ON ".DB_PREFIX."playlists.owner = ".DB_PREFIX."users.id order by views DESC limit 0,1000");
	
	$plx = array("" => "v/m","1" => "v/m","2" => "pics");
	if($p) {
	foreach($p as $pl)  { ?>
	<option value="<?php echo $pl->id;?>"><?php echo '[ '.$plx[$pl->ptype].' ]   '._html($pl->title);?> by <?php echo _html($pl->user);?> </option>
	<?php }
	}
	?>
	</select>
