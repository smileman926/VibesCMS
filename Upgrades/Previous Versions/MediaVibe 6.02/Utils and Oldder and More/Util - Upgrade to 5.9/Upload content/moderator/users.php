...
<th> <div class="checkbox-custom checkbox-danger"> <input type="checkbox" name="checkRows" class="check-all" /> <label for="checkRows"></label> </div>  </th>
						  ...
						  <?php foreach ($users as $video) { ?>
                              <tr>
								  ...
								  <a class="btn btn-sm btn-primary" href="<?php echo admin_url('edit-user');?>&id=<?php echo $video->id;?>"><i class="icon icon-edit"></i><?php echo _lang("Edit"); ?></a>
								 <?php 
if(!_contains($video->pass, 'banned')) { ?>
								 <a class="btn btn-sm btn-default" href="<?php echo admin_url('users');?>&p=<?php echo this_page();?>&ban=<?php echo $video->id;?>"><i class="icon icon-exclamation-triangle"></i><?php echo _lang("Ban"); ?></a>
<?php } else { ?>  
<a class="btn btn-sm btn-success" href="<?php echo admin_url('users');?>&p=<?php echo this_page();?>&removeban=<?php echo $video->id;?>"><i class="icon icon-exclamation-circle"></i><?php echo _lang("Remove Ban"); ?></a>


<?php } ?>  
								  <a class="btn btn-sm btn-danger confirm" href="<?php echo admin_url('users');?>&p=<?php echo this_page();?>&delete-user=<?php echo $video->id;?>"><i class="icon icon-eraser"></i><?php echo _lang("Delete"); ?></a>
