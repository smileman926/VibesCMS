...
$(document).ready(function() {
    ...
    $.getJSON(site_url + "api/noty/", function(data) {
        if (data) {
            ...
            if(data.buzz) {
            $("li.my-buzz > a").append('<span class="badge badge-info pull-right">'+ data.buzz + '</span>');
			}
    ...
    $(".fullit").click(function() {
        ...
        $("#flT").toggleClass('icon-arrows-alt').toggleClass('icon-compress');
    });
    //Kill ad
...
function addEMComment(oid, toid) {
    if ($('textarea#addEmComment_' + toid).val()) {
        $.post(
            ...
            function(data) {
                $('#emContent_' + oid + '-' + toid + ' li:first').after('<li id="comment-' + data.id + '" class="left animated rollIn"><img class="avatar" src="' + data.image + '" /><div class="message"><span class="arrow"> </span><a class="name" href="' + data.url + '">' + data.name + '</a> <span class="date-time"> ' + data.date + ' </span> <div class="body">' + data.text + '</div> </div></li>');
